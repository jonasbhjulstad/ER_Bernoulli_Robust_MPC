...
struct cl::sycl::is_device_copyable<
    Sycl_Graph::Sycl::Network_Models::SIR_Individual_State>: std::true_type{};
...
// make SIR_Invididual_State device copyable
struct SIR_Bernoulli_Network
  ...
  SIR_Bernoulli_Network(Graph_t &G, float p_I0, float p_R0,
                        uint32_t N_neighbors_max = 1, int seed = 777)
      : Base_t(3), q(G.q), G(G), p_I0(p_I0), p_R0(p_R0),
        N_neighbors_max(N_neighbors_max), seed_buf(sycl::range<1>(G.NV)) {
    assert(G.NV > 0);
    ...
    q.submit([&](sycl::handler &h) {
      ...
      h.parallel_for(sycl::range<1>(G.NV),
                     [=](sycl::id<1> i) { seed[i] = seeds[i]; });
    });
  }

  SIR_Bernoulli_Network operator=(const SIR_Bernoulli_Network &other) {
  ...
  void infection_step(float p_I) {
    ...
    auto sus_inf_edge_comp([&](auto &v_acc, const uint32_t e0_to,
                               const uint32_t e0_from, const uint32_t e1_to,
                               const uint32_t e1_from) {
      // if invalid
      ...
      bool e0_valid =
          (v0_data[0] == SIR_INDIVIDUAL_S && v0_data[1] == SIR_INDIVIDUAL_I) ||
          (v0_data[0] == SIR_INDIVIDUAL_I && v0_data[1] == SIR_INDIVIDUAL_S);
      bool e1_valid =
          (v1_data[0] == SIR_INDIVIDUAL_S && v1_data[1] == SIR_INDIVIDUAL_I) ||
          (v1_data[0] == SIR_INDIVIDUAL_I && v1_data[1] == SIR_INDIVIDUAL_S);

      if (e0_valid && e1_valid) {
        uint32_t e0_sus_idx =
            (v0_data[0] == SIR_INDIVIDUAL_S) ? e0_from : e0_to;
        uint32_t e1_sus_idx =
            (v1_data[0] == SIR_INDIVIDUAL_S) ? e1_from : e1_to;
    ...
    q.submit([&, this](sycl::handler &h) {
      ...
      h.parallel_for<class edge_validity>(
          cl::sycl::range<1>(e_acc.to.size()), [=](cl::sycl::id<1> index) {
            // get the index of the element to sort
            int i = index[0];
            uint32_t sn = get_susceptible_neighbor(v_acc.data, e_acc.to[i],
                                                   e_acc.from[i]);
  ...
  bool terminate(const SIR_Bernoulli_Param<> &p,
...
template <>
struct sycl::is_device_copyable<Sycl_Graph::Sycl::Network_Models::SIR_Bernoulli_Network::Edge_t>: std::true_type {};
#endif
#endif